...
class PoseMatcher():
    ...
    def match(self, pts_out_path, dep_out_path, gt_dep_path):
        ...
        vid_results = sorted(glob.glob(os.path.join(self.top_down_path, '*.pkl')), key=lambda s: int(s.split('/')[-1].split('.')[0]))
        ...
        for v in tqdm(vid_results):
            ...
            results = []
            depths = []
            lastNonemptyBu_pts = None #hack non-empty since bottom up returning empty sometimes
            lastNonemptyBu_depths = None #hack non-empty since bottom up returning empty sometimes
            for frame in range(len(pts)):
                bu_pts = bu_estimations[vid_idx][frame][3]
                if len(bu_pts) == 0:
                    bu_pts = lastNonemptyBu_pts
                else:
                    lastNonemptyBu_pts = bu_pts

                bu_pts = np.float32(bu_pts).transpose([0,2,1])
                bu_depths = np.float32(bu_estimations[vid_idx][frame][1])
                if len(bu_depths) == 0:
                    bu_depths = lastNonemptyBu_depths
                else:
                    lastNonemptyBu_depths = bu_depths
            ...
            depths = (depths[None, ...])[0]*1000 #remove procrustes analysis. scale by 1000, since that's essentially what the procrustes is doing?
