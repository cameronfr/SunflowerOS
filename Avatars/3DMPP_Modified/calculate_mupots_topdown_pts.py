...
if __name__=='__main__':
        ...
        for ptsfile in sorted(glob.glob('mupots/est_p2ds/*Custom*.pkl')):
                ptsfile = ptsfile.replace('\\','/') # for windows
                print("Processing", ptsfile)
                ...
                affb = (torch.ones(p2d.shape[0],16,16) / 16).cuda()
                affpts = (torch.ones(p2d.shape[0],17,17) / 17).cuda()
                # occmask = torch.from_numpy([]).cuda()
                with torch.no_grad():
