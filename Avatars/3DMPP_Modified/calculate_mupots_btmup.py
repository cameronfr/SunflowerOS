...
if __name__=='__main__':
        ...
        with torch.no_grad():
                ...
                res_all = {}
                for i in range(1):#I changed from 20 to 1
                        print('Seq:',i)
                        # imgs = sorted(glob.glob('./MultiPersonTestSet/TS%d/*.jpg'%(i+1)))
                        imgs = sorted(glob.glob('./images_store_tmp/*.png'), key=lambda s: int(s.split("/")[-1].split(".")[0]))
                        buff = []
                        # print(imgs)
                        for frame_idx,imgname in enumerate(tqdm(imgs)):
